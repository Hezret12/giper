...
function Brand() {
  ...
  const fetchData = async () => {
    ...
    const response = await axios.get(`https://gipertm.com/api/v1/manufacturer?search=&offset=${totalPages}&max=40&lang=tk`);
  ...
  return (
    ...
    <div className='mt-4 mb-2'>
        <h3 className='text-gray-800 text-2xl font-bold'>Brand</h3>
    </div>
    <div className='grid grid-cols-3 mb-4'>
      ...
      >
        ...
        {characters.map((character, index) => (
          <BrandCart key={index} logo={character} />
        ))}
        </div>
      </InfiniteScroll>
    </div>
